...
function renderMarkdown(text) {
  ...
  // Process headers
  text = text.replace(/^### (.*$)/gm, '<h3 class="text-lg font-bold mt-4 mb-2 text-gray-800 dark:text-gray-200">$1</h3>');
  ...
  text = text.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-primary-600 dark:text-primary-400 hover:underline" target="_blank">$1</a>');
...
function generateStarRating(rating) {
  ...
  for (let i = 0; i < Math.floor(rating); i++) {
    html += '<svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  }
  ...
  for (let i = Math.ceil(rating); i < 5; i++) {
    html += '<svg class="w-5 h-5 text-gray-300 dark:text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  }
  ...
  try {
    ...
    if (longDescriptionEl) {
      if (agent.long_description) {
        ...
        longDescriptionEl.innerHTML = '<p class="text-gray-500 dark:text-gray-400">No detailed description available.</p>';
      }
    }
    ...
    if (featuresList && agent.features && Array.isArray(agent.features) && agent.features.length > 0) {
      featuresList.innerHTML = agent.features.map(feature => {
      ...
      if (agent.setup_instructions) {
        if (typeof agent.setup_instructions === 'object' && agent.setup_instructions !== null) {
          if (agent.setup_instructions.docker) {
            setupContent += '<h3>Docker Setup</h3>\n' + renderMarkdown(agent.setup_instructions.docker) + '\n<hr class="my-4">\n';
          }
          if (agent.setup_instructions.python) {
            setupContent += '<h3>Python Setup</h3>\n' + renderMarkdown(agent.setup_instructions.python);
          }
          if (Object.keys(agent.setup_instructions).length === 0 || (!agent.setup_instructions.docker && !agent.setup_instructions.python && agent.setup_instructions.general)) {
            setupContent = renderMarkdown(agent.setup_instructions.general || 'No specific setup instructions provided.');
    ...
    if (agent.llm_dependency) {
      ...
      setText('llm-api-key-env-var', agent.llm_dependency.api_key_env_var ? `<code>${agent.llm_dependency.api_key_env_var}</code>` : null);
    ...
    if (agent.docker_info) {
      setText('docker-image-name', agent.docker_info.image_name ? `<code>${agent.docker_info.image_name}</code>` : null);
    ...
    if (configForm && agent.configFields && Array.isArray(agent.configFields) && agent.configFields.length > 0) {
      ...
      const formHtml = agent.configFields.map(field => {
    ...
    if (Array.isArray(agent.use_cases)) {
      populateList('agent-use-cases', agent.use_cases, 
      ...
      if (el) el.innerHTML = '<li><span class="text-gray-400 dark:text-gray-500">Use cases not available or in unexpected format.</span></li>';
    }

    // Requirements
    if (Array.isArray(agent.requirements)) {
      populateList('agent-requirements', agent.requirements, 
      ...
      if (el) el.innerHTML = '<li><span class="text-gray-400 dark:text-gray-500">Requirements not available or in unexpected format.</span></li>';
    }

    // Roadmap Features
    if (Array.isArray(agent.roadmap_features)) {
      populateList('agent-roadmap-features', agent.roadmap_features, 
      ...
      if (el) el.innerHTML = '<li><span class="text-gray-400 dark:text-gray-500">Roadmap features not available or in unexpected format.</span></li>';
    }
    ...
    if (agent.docker_info) {
      ...
      setText('docker-pull-instructions-content', '<p class="text-gray-400 dark:text-gray-500">No Docker pull instructions provided.</p>');
      setText('docker-run-instructions-content', '<p class="text-gray-400 dark:text-gray-500">No Docker run instructions provided.</p>');
    }
